...
describe("Rate limiter", () => {
  describe("checkRateLimit", () => {
    ...
    it("uses a separate counter per key", async () => {
      ...
      const r1 = await checkRateLimit(key1, limit, 60);
      const r2 = await checkRateLimit(key2, limit, 60);
    ...
    it("resets the window after the window duration", async () => {
      ...
      await checkRateLimit(key, limit, windowSecs); // use up the limit
      const blocked = await checkRateLimit(key, limit, windowSecs);
      expect(blocked.allowed).toBe(false);

      await new Promise((r) => setTimeout(r, 60)); // wait for window to expire

      const reset = await checkRateLimit(key, limit, windowSecs);
  ...
  describe("purgeExpiredEntries", () => {
    it("removes entries that have exceeded the window age", async () => {
      ...
      await checkRateLimit(key, 10, 0.01); // 10ms window
