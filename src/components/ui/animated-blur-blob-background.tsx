...
const AnimatedBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const randomX = (direction = 1) =>
      (Math.random() * 800 - 400) * direction;
    const randomY = (direction = 1) =>
      (Math.random() * 400 - 200) * direction;
    ...
    blurElements.forEach((blur) => {
      if (blur) {
        blur.style.transform = `translate(${randomX(-1)}px, ${randomX(
          1,
        )}px) rotate(${randomAngle(-1)}deg)`;
      }
    });
    ...
    const rotate = (target: HTMLElement, direction: number) => {
      ...
      const animateRotation = (timestamp: number) => {
        ...
        if (progress < 1) {
          ...
          setTimeout(() => !cancelled && rotate(target, direction * -1), 0);
        }
      };
    ...
    const moveX = (target: HTMLElement, direction: number) => {
      ...
      const animateX = (timestamp: number) => {
        ...
        if (progress < 1) {
          ...
          setTimeout(() => !cancelled && moveX(target, direction * -1), 0);
        }
      };
    ...
    const moveY = (target: HTMLElement, direction: number) => {
      ...
      const animateY = (timestamp: number) => {
        ...
        if (progress < 1) {
          ...
          setTimeout(() => !cancelled && moveY(target, direction * -1), 0);
        }
      };
  ...
  return (
    <section className="wrapper" aria-hidden="true">
