...
function MiniSummary({ report }: { report: ImportSummaryReport }) {
  ...
  const created = created_counts.skills + created_counts.agents + created_counts.files + created_counts.notes;
  ...
  return (
    <div className="flex flex-col gap-2 rounded-md border border-border bg-muted/20 p-4">
      ...
      <div className="flex flex-wrap gap-3">
        {created_counts.skills > 0 && (
          <span className="text-xs text-muted-foreground">
            <span className="font-semibold text-foreground">{created_counts.skills}</span> skill{created_counts.skills !== 1 ? "s" : ""} created
          </span>
        )}
        {created_counts.notes > 0 && (
          <span className="text-xs text-muted-foreground">
            <span className="font-semibold text-foreground">{created_counts.notes}</span> note{created_counts.notes !== 1 ? "s" : ""} created
          </span>
        )}
        {created === 0 && (
          <span className="text-xs text-muted-foreground">No new objects created</span>
        )}
      </div>
      {warnings.length > 0 && (
...
/**
 * Dialog for importing a skill package into the workspace library.
 * Accepts:
 *   - A .zip package (v1.1 manifest with skill entries) — imports skills with full metadata
 *   - Redirects to /app/skills after successful import
 */
export function SkillImportDialog({ onClose }: SkillImportDialogProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col gap-4 rounded-xl border border-border bg-background p-6 shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-foreground">Import skill package</h2>
            <p className="text-xs text-muted-foreground mt-0.5">
              Import a packaged .zip into the workspace skill library
            </p>
          </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex gap-2">
                {(["create_copy", "replace_by_id", "remap_ids_and_import"] as CollisionMode[]).map((mode) => (
                  ...
                  >
                    <input type="radio" name="skill_collision_mode" value={mode} checked={collisionMode === mode} onChange={() => setCollisionMode(mode)} className="sr-only" />
...
export function SkillImportTrigger() {
  ...
  return (
    <>
      ...
      {open && <SkillImportDialog onClose={() => setOpen(false)} />}
    </>
  );
}
